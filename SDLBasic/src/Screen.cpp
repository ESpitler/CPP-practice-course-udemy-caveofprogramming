...
bool Screen::init() {
	...
	//buffer[30000] = 0xFFC500FF;

	/*for (int i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++) {
		m_buffer[i] = 0xFFC500FF;
	}*/
...
void Screen::boxBlur(){
	...
	for(int y = 0; y < SCREEN_HEIGHT; y++){
		for(int x = 0; x< SCREEN_WIDTH; x++){
			 ...
			 * 0 1 0
			 * 0 0 0
			 * */ //the nine cells surrounding the target pixel
			...
			for(int row = -1; row <= 1; row++){
				for(int col = -1; col <= 1; col++){
					...
					if(currentX >= 0 && currentX < SCREEN_WIDTH && currentY >= 0 && currentY < SCREEN_HEIGHT){//if within the screen bounds
